...
const AP_Param::GroupInfo AP_MotorsHeli_Single::var_info[] = {
    ...
    // @Param: AILERON_COMP
    // @DisplayName: Aileron to Collective Mixing
    // @Description: Mixes collective with aileron input to tune out interaction
    // @Range: -10 10
    // @Units: Percent
    // @User: Advanced
    // @Increment: 1
    AP_GROUPINFO("AILERON_COMP", 20, AP_MotorsHeli_Single, _aileron_comp, 0),

    // @Param: ELEVATOR_COMP
    // @DisplayName: Elevator to Collective Mixing
    // @Description: Mixes collective with elevator input to tune out interaction
    // @Range: -20 20
    // @Units: Percent
    // @User: Advanced
    // @Increment: 1
    AP_GROUPINFO("ELEVATOR_COMP", 21, AP_MotorsHeli_Single, _elevator_comp, 0),
...
{   //Three-Servo adjustable CCPM mixer factors
    ...
    //Three-Servo H3-140 CCPM mixer factors
        ...
        _rollFactor[CH_1] = 1;
        _rollFactor[CH_2] = -1;
        _rollFactor[CH_3] = 0 + (_aileron_comp * 0.01f);

        // elevator factors
        if (_elevator_comp >=0) {
            _pitchFactor[CH_1] = 1 - (_elevator_comp * 0.01f);
            _pitchFactor[CH_2] = 1 - (_elevator_comp * 0.01f);
            _pitchFactor[CH_3] = -1;
        } else {
            _pitchFactor[CH_1] = 1;
            _pitchFactor[CH_2] = 1;
            _pitchFactor[CH_3] = -1 - (_elevator_comp * 0.01f);
        }
...
bool AP_MotorsHeli_Single::parameter_check(bool display_msg) const
{
    ...
    // returns false if Aileron Compensation is outside of range
    if ((_aileron_comp >10) || (_aileron_comp < -10)){
        if (display_msg) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "PreArm: H_AILERON_COMP out of range");
        }
        return false;
    }

    // returns false if Elevator Compensation is outside of range
    if ((_elevator_comp >20) || (_elevator_comp < -20)){
        if (display_msg) {
            GCS_MAVLINK::send_statustext_all(MAV_SEVERITY_CRITICAL, "PreArm: H_ELEVATOR_COMP out of range");
        }
        return false;
    }
